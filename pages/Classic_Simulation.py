...
MODELS_DIR = Path(__file__).parent.parent / "models" / "v0.1.0"
...
with st.sidebar:
    st.title("🚢  Manifest  (v0.1.0)")
    st.caption("Classic simulation — balance-only objective, no port-stop ordering. Configure in sidebar, then click **Run**.")
    st.divider()
...
st.header("Classic Simulation  ·  v0.1.0", divider="gray")
st.caption(
...
with prev_col:
    ...
    st.plotly_chart(plot_hull_preview(ship_cfg), width="stretch")
...
with dist_col:
    ...
    if containers_preview:
        st.plotly_chart(plot_weight_dist(containers_preview), width="stretch")
...
if results:
    st.plotly_chart(plot_summary_comparison(results), width="stretch")
...
for i, (name, (manifest, ship, stats)) in enumerate(results.items()):
    ...
    with col:
        ...
        st.plotly_chart(plot_solver_metrics(stats, name), width="stretch")
